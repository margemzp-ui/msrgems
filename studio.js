(function () {
  ...
  function setFloatingWindowUi(floating) { els.detachStudio.textContent = floating ? 'Move using title bar' : 'Detach studio'; els.detachStudio.classList.toggle('floating', floating); els.detachStudio.disabled = floating; els.detachStudio.title = floating ? 'Drag the native window title bar to move Facescreen Recorder anywhere on your desktop.' : 'Move this Studio tab into its own draggable Chrome window.'; }
  function detectWindowMode() { chrome.windows.getCurrent(function (windowInfo) { setFloatingWindowUi(windowInfo.type === 'popup'); }); }
  function detachStudio() { if (state.recorder && state.recorder.state !== 'inactive') { showError(new Error('Stop the recording before changing Studio window mode.')); return; } els.detachStudio.disabled = true; els.detachStudio.textContent = 'Detaching…'; chrome.runtime.sendMessage({ type: 'detach-studio' }, function (response) { var runtimeMessage = chrome.runtime.lastError && chrome.runtime.lastError.message; if (runtimeMessage && /message port closed|receiving end does not exist/i.test(runtimeMessage)) { setTimeout(detectWindowMode, 300); return; } if (runtimeMessage || (response && response.ok === false)) { els.detachStudio.disabled = false; els.detachStudio.textContent = 'Detach studio'; showError(new Error((response && response.error) || runtimeMessage || 'Could not detach Studio.')); return; } setTimeout(detectWindowMode, 100); }); }
  function rawQuality() {
  ...
  function quality() { var requested = rawQuality(), profile = els.performanceProfile.value, maxHeight = Infinity, maxFps = Infinity, maxBitrate = Infinity; if (profile === 'economy') { maxHeight = 720; maxFps = 24; maxBitrate = 4000000; } else if (profile === 'balanced') { maxHeight = 1080; maxFps = 30; maxBitrate = 8000000; } else if (profile === 'auto') { if (state.deviceTier === 'low') { maxHeight = 720; maxFps = 24; maxBitrate = 4000000; } else if (state.deviceTier === 'balanced') { maxHeight = 1080; maxFps = 30; maxBitrate = 8000000; } } var height = Math.min(requested.height, maxHeight), fps = Math.min(requested.fps, maxFps), bitrate = Math.min(requested.bitrate, maxBitrate); return { height: height, fps: fps, bitrate: bitrate, width: height === 480 ? 640 : Math.round(height * 16 / 9), capped: height !== requested.height || fps !== requested.fps || bitrate !== requested.bitrate }; }
  function updatePerformanceStatus() { var device = detectDeviceTier(), actual = quality(), label = device.tier === 'low' ? 'Low-resource device profile' : device.tier === 'high' ? 'High-capacity device profile' : 'Balanced device profile'; els.performanceStatus.textContent = label; els.performanceStatusDot.classList.toggle('warn', actual.capped); els.performanceDetails.textContent = (actual.capped ? 'Settings safely capped at ' : 'Configured for ') + actual.height + 'p · ' + actual.fps + ' FPS · ' + Math.round(actual.bitrate / 1000000) + ' Mbps. ' + device.cores + ' logical CPU cores detected.'; document.documentElement.classList.toggle('reduced-effects', els.performanceProfile.value === 'economy' || (els.performanceProfile.value === 'auto' && device.tier === 'low')); }
  ...
  async function updateHardwareAccelerationStatus() {
    ...
    if (navigator.mediaCapabilities && navigator.mediaCapabilities.encodingInfo) { try { info = await navigator.mediaCapabilities.encodingInfo({ type: 'record', video: { contentType: 'video/webm;codecs=' + codec, width: q.width, height: q.height, bitrate: q.bitrate, framerate: q.fps } }); } catch (error) { console.debug('Encoding capability details unavailable', error); } }
  ...
  function renderFrame(timestamp) {
    var recording = state.recorder && state.recorder.state === 'recording', targetFps = recording ? quality().fps : state.previewOnly ? 15 : 8; if (performance.now() < state.adaptiveUntil) targetFps = Math.min(targetFps, 15); var interval = 1000 / targetFps; var now = timestamp || performance.now();
    ...
    var cost = performance.now() - renderStarted; state.renderCost = state.renderCost ? state.renderCost * .9 + cost * .1 : cost; if (recording && state.renderCost > interval * .72 && performance.now() > state.adaptiveUntil) { state.adaptiveUntil = performance.now() + 2500; els.performanceStatus.textContent = 'Reducing preview load temporarily'; els.performanceStatusDot.classList.add('warn'); setTimeout(updatePerformanceStatus, 2700); }
  ...
  function setMode(mode) {
    ...
    els.cameraField.hidden = mode === 'screen'; $('cameraReadiness').hidden = mode === 'screen'; els.pipSection.hidden = mode !== 'pip'; els.previewCamera.hidden = mode === 'screen'; els.canvas.classList.toggle('pip-draggable', mode === 'pip');
    if (mode === 'pip') chrome.storage.local.get('cameraCoachSeen', function (result) { if (!result.cameraCoachSeen) els.coach.hidden = false; }); else els.coach.hidden = true; savePrefs();
  }
  function setControls(recording) {
    els.start.hidden = recording; els.stop.hidden = !recording; els.pause.hidden = !recording;
    els.previewCamera.hidden = recording || state.mode === 'screen'; els.floatCamera.hidden = !state.cameraStream;
  ...
  async function startCameraPreview() {
    ...
    try {
      setStatus('Requesting camera preview', 'busy'); var q = quality(); state.cameraStream = await navigator.mediaDevices.getUserMedia({ video: { deviceId: els.cameraSelect.value ? { exact: els.cameraSelect.value } : undefined, width: { ideal: q.width }, height: { ideal: q.height }, frameRate: { ideal: q.fps } }, audio: false }); await attachStream(els.cameraVideo, state.cameraStream); state.previewOnly = true; state.cameraOn = true; els.quickCamera.setAttribute('aria-pressed', 'true'); els.quickCamera.textContent = 'Camera on'; setReady(els.cameraReady, 'Ready', 'ready'); setReady(els.framing, 'Adjust in preview', 'ready');
      ...
      els.empty.hidden = true; els.quickBar.hidden = false; els.previewCamera.textContent = 'Stop camera preview'; els.floatCamera.hidden = false; els.snapshot.disabled = false; startLightingCheck(); setStatus('Camera preview ready', 'ready'); setSession('Check framing and audio');
    } catch (error) { setReady(els.cameraReady, error.name === 'NotAllowedError' ? 'Blocked' : 'Unavailable', 'warn'); showError(new Error(error.name === 'NotAllowedError' ? 'Camera access is blocked. Allow it from Chrome\'s camera permission icon and try again.' : 'Camera preview failed: ' + error.message)); }
  }
  function stopCameraPreview() { if (state.recorder && state.recorder.state !== 'inactive') return; [state.cameraStream, state.micStream].forEach(function (stream) { if (stream) stream.getTracks().forEach(function (track) { track.stop(); }); }); state.cameraStream = state.micStream = null; state.previewOnly = false; els.cameraVideo.srcObject = null; clearInterval(state.lightingTimer); state.analyser = null; cancelAnimationFrame(state.meterFrame); if (state.audioCtx) state.audioCtx.close().catch(function () {}); state.audioCtx = null; els.quickBar.hidden = true; els.floatCamera.hidden = true; els.previewCamera.textContent = 'Preview camera'; els.empty.hidden = false; els.snapshot.disabled = true; setReady(els.cameraReady, 'Not checked', ''); setReady(els.micReady, 'Not checked', ''); setReady(els.lighting, 'Not checked', ''); setReady(els.framing, 'Use live preview', ''); setStatus('Ready to capture', 'ready'); setSession('Not started'); }
  async function acquireSources() {
    ...
    if (state.mode !== 'camera') {
      ...
      try {
        state.displayStream = await navigator.mediaDevices.getDisplayMedia({ video: { frameRate: q.fps }, audio: true });
      } catch (error) {
    ...
    if ((state.mode === 'camera' || (state.mode === 'pip' && els.pipEnabled.checked)) && !state.cameraStream) {
      ...
      try {
        ...
        if (error.name === 'NotAllowedError') throw new Error('Camera access is blocked. Open Chrome Settings > Privacy and security > Site settings > Camera, allow Facescreen Recorder, then try again.');
  ...
  function buildRecordingStream() {
    var q = quality(), canvasStream = els.canvas.captureStream(q.fps), audioTracks = [];
    ...
    if (audioTracks.length) {
      if (!state.audioCtx) state.audioCtx = new AudioContext(); var destination = state.audioCtx.createMediaStreamDestination();
      audioTracks.forEach(function (track) { var isolated = new MediaStream([track]); state.audioCtx.createMediaStreamSource(isolated).connect(destination); });
      destination.stream.getAudioTracks().forEach(function (track) { canvasStream.addTrack(track); });
    }
    return canvasStream;
  }
  function mimeType() {
  ...
  async function startRecording() {
    try {
      ...
      await acquireSources(); await countdown();
      ...
      chrome.storage.local.set({ activeRecording: { sessionId: state.writer ? opfsName : '', startedAt: Date.now(), mode: state.mode, quality: quality() } });
    ...
    } catch (error) { cleanupStreams(); setControls(false); showError(error); }
  }
  ...
  function togglePause() {
    ...
    if (state.recorder.state === 'recording') { state.recorder.pause(); state.pausedAt = Date.now(); els.pause.textContent = 'Resume'; setSession('Paused'); setStatus('Recording paused', 'busy'); }
    else if (state.recorder.state === 'paused') { state.recorder.resume(); state.pausedTotal += Date.now() - state.pausedAt; state.pausedAt = 0; els.pause.textContent = 'Pause'; setSession('Recording'); setStatus('Recording in progress', 'recording'); }
  }
  ...
  async function finishRecording() {
    ...
    cleanupStreams(); setControls(false); els.recordBadge.hidden = true; els.stop.disabled = false; els.pause.disabled = false; els.pause.textContent = 'Pause';
    try { blob = await finalizeWriter(); } catch (error) { chrome.storage.local.remove('activeRecording'); showError(new Error('The recording could not be finalized: ' + error.message)); state.stopping = false; return; }
    if (!blob.size) { chrome.storage.local.remove('activeRecording'); showError(new Error('The recording was empty. Please try again.')); state.stopping = false; return; }
    chrome.storage.local.remove('activeRecording'); if (state.lastBlobUrl) URL.revokeObjectURL(state.lastBlobUrl); state.lastBlobUrl = URL.createObjectURL(blob); state.lastFilename = 'Facescreen Recorder/Recordings/' + filename('webm');
  ...
  function downloadLast() { if (state.lastBlobUrl) chrome.downloads.download({ url: state.lastBlobUrl, filename: state.lastFilename, saveAs: false }); }
  function snapshot() { $('previewCard').classList.add('snapshot-flash'); setTimeout(function () { $('previewCard').classList.remove('snapshot-flash'); }, 350); els.canvas.toBlob(function (blob) { if (!blob) return; var url = URL.createObjectURL(blob); chrome.downloads.download({ url: url, filename: 'Facescreen Recorder/Snapshots/' + filename('png'), saveAs: false }, function () { setTimeout(function () { URL.revokeObjectURL(url); }, 30000); }); }, 'image/png'); }
  ...
  async function recoverInterruptedRecording() {
    chrome.storage.local.get('activeRecording', async function (result) {
      var active = result.activeRecording; if (!active) return;
      if (!active.sessionId || !window.indexedDB) { chrome.storage.local.remove('activeRecording'); setStatus('Previous recording was interrupted before it could be recovered', 'error'); return; }
      try {
        ...
        setStatus('Interrupted recording is ready to download', 'ready'); setSession('Recovered recording'); chrome.storage.local.remove('activeRecording');
      } catch (error) { console.warn('Recovery failed', error); chrome.storage.local.remove('activeRecording'); setStatus('Previous recording could not be recovered', 'error'); }
    });
  }
  ...
  async function openDocumentFloatingCamera() { var size = floatingCameraSize(); var pipWindow = await documentPictureInPicture.requestWindow({ width: size.width, height: size.height, disallowReturnToOpener: false }); state.documentPipWindow = pipWindow; var style = pipWindow.document.createElement('style'); style.textContent = 'html,body{width:100%;height:100%;margin:0;overflow:hidden;background:#05070b}#floatingCameraShell{width:100%;height:100%;overflow:hidden;background:#05070b;box-shadow:inset 0 0 0 2px rgba(96,165,250,.75)}video{display:block;width:100%;height:100%;object-fit:cover}'; var shell = pipWindow.document.createElement('div'); shell.id = 'floatingCameraShell'; var video = pipWindow.document.createElement('video'); video.id = 'floatingCameraVideo'; video.autoplay = true; video.muted = true; video.playsInline = true; video.srcObject = state.cameraStream; shell.appendChild(video); pipWindow.document.head.appendChild(style); pipWindow.document.body.appendChild(shell); await video.play(); updateFloatingCameraAppearance(false); pipWindow.addEventListener('pagehide', function () { state.documentPipWindow = null; syncFloatingCameraState(false); }); syncFloatingCameraState(true); }
  async function toggleFloatingCamera() { try { if (state.documentPipWindow && !state.documentPipWindow.closed) { state.documentPipWindow.close(); return; } if (document.pictureInPictureElement) { await document.exitPictureInPicture(); return; } if (!state.cameraStream) return; if ('documentPictureInPicture' in window) await openDocumentFloatingCamera(); else await els.cameraVideo.requestPictureInPicture(); } catch (error) { showError(new Error('Floating camera is unavailable: ' + error.message)); } }
  function syncFloatingCameraState(active) { els.floatCamera.textContent = active ? 'Return camera to preview' : 'Open floating camera'; els.floatCamera.classList.toggle('active', active); }
  ...
  function savePrefs() { chrome.storage.local.set({ recorderPrefs: { mode: state.mode, mic: els.micEnabled.checked, quality: els.quality.value, resolution: els.resolution.value, fps: els.fps.value, bitrate: els.bitrate.value, pipEnabled: els.pipEnabled.checked, mirrorCamera: els.mirrorCamera.checked, pipCustom: state.pipCustom, shape: els.pipShape.value, position: els.pipPosition.value, size: els.pipSize.value, preview: els.previewEnabled.checked, hardwareAcceleration: els.hardwareAcceleration.checked, performanceProfile: els.performanceProfile.value } }); }
  ...
  function loadPrefs() { chrome.storage.local.get('recorderPrefs', function (result) { var p = result.recorderPrefs || {}; if (p.quality) els.quality.value = p.quality; if (p.resolution) els.resolution.value = p.resolution; if (p.fps) els.fps.value = p.fps; if (p.bitrate) els.bitrate.value = p.bitrate; if (p.shape) els.pipShape.value = p.shape; if (p.position) els.pipPosition.value = p.position; if (p.size) els.pipSize.value = p.size; if (typeof p.pipEnabled === 'boolean') els.pipEnabled.checked = p.pipEnabled; if (typeof p.mirrorCamera === 'boolean') els.mirrorCamera.checked = p.mirrorCamera; if (p.pipCustom && Number.isFinite(p.pipCustom.x) && Number.isFinite(p.pipCustom.y)) state.pipCustom = p.pipCustom; if (typeof p.mic === 'boolean') els.micEnabled.checked = p.mic; if (typeof p.preview === 'boolean') els.previewEnabled.checked = p.preview; if (typeof p.hardwareAcceleration === 'boolean') els.hardwareAcceleration.checked = p.hardwareAcceleration; if (p.performanceProfile) els.performanceProfile.value = p.performanceProfile; els.pipSizeValue.textContent = els.pipSize.value + '%'; els.micField.hidden = !els.micEnabled.checked; updatePipControls(); syncMirrorButton(); $('previewCard').classList.toggle('preview-off', !els.previewEnabled.checked); setMode(p.mode || 'screen'); updatePerformanceStatus(); setCanvasSize(); updateHardwareAccelerationStatus(); }); }
  ...
  els.start.addEventListener('click', startRecording); els.stop.addEventListener('click', stopRecording); els.pause.addEventListener('click', togglePause); els.previewCamera.addEventListener('click', startCameraPreview); els.snapshot.addEventListener('click', snapshot); els.floatCamera.addEventListener('click', toggleFloatingCamera);
  ...
  els.dismissCoach.addEventListener('click', function () { els.coach.hidden = true; chrome.storage.local.set({ cameraCoachSeen: true }); });
  ...
  els.clearHistory.addEventListener('click', function () { chrome.storage.local.remove('recordingHistory', function () { renderHistory([]); }); });
  els.detachStudio.addEventListener('click', detachStudio);
  els.reset.addEventListener('click', function () { chrome.storage.local.remove('recorderPrefs', function () { location.reload(); }); });
  ...
  paintEmpty(); renderFrame(); detectWindowMode(); loadPrefs(); listDevices(); loadHistory(); recoverInterruptedRecording();
})();
